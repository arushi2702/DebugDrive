...
import { BugContext } from '../types/agent';
...
export function registerDebugDriveCommands(context: vscode.ExtensionContext): void {
  ...
  const runSessionCommand = vscode.commands.registerCommand('debug-drive.runDebugSession', async () => {
    ...
    const learningStore = new LearningStore(path.join(repositoryPath, '.debug-drive-memory'));
    const experimentStore = new ExperimentStore(path.join(repositoryPath, '.debug-drive-memory'));
    ...
    const rankedEmbeddingRecords = retrievalStore
      .searchEmbeddingRecords(vectorizer.embedText(queryEmbeddingText), RETRIEVAL_TOP_K)
    ...
    const decision = await coordinator.runSession(session, retrievalRecords);
    ...
    if (decision.nextAction === 'accept' && session.latestPatch) {
      ...
      retrievalRecordSaved = retrievalStore.appendRecord(retrievalRecord);
        ...
        retrievalStore.appendEmbeddingRecord({
          ...
          text: embeddingText,
          embedding: vectorizer.embedText(embeddingText),
          metadata: {
  ...
  context.subscriptions.push(runSessionCommand, outputChannel);
}
